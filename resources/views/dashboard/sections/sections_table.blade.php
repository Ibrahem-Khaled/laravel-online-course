<div class="table-responsive p-0">
    <!-- أنماط مخصصة لحجم الصور وتأثير المرور -->
    <style>
        .avatar-sm {
            width: 40px;
            height: 40px;
    ...
    <table class="table table-sm table-striped table-hover align-items-center mb-0">
        <thead>
            <tr>
                ...
                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">رابط الاجتماع
                </th>
                ...
                <tr>
                        ...
                        <div class="d-flex px-2 py-1 align-items-center">
                            @if ($section->image)
                                <img src="{{ asset('storage/' . $section->image) }}"
                                    class="avatar-sm rounded-circle me-3" alt="صورة القسم">
                            @else
                                <div class="avatar-placeholder bg-gradient-primary me-3">
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <p class="text-xs text-secondary mb-0">
                                    {{ Str::limit($section->description, 30) }}
                                </p>
                            </div>
                        </div>
                    </td>
                    <td class="text-white">
                        ...
                        @else bg-gradient-danger @endif">
                            {{ $section->type == 'ambitious_program'
                                ? 'البرنامج الطموح 1'
                                : ($section->type == 'ambitious_program2'
                                    ? 'البرنامج الطموح 2'
                                    : 'ريادة الأعمال') }}
                        </span>
                    </td>
                    ...
                    <td class="align-middle text-center text-sm text-white">
                        <span class="badge bg-gradient-warning">
                            {{ $section->users()->where('role', 'teacher')->count() }}
                        </span>
                    </td>
                    <td class="align-middle text-center text-sm text-white">
                        <span class="badge bg-gradient-info">
                            {{ $section->courses_count }}
                        </span>
                    </td>
                    <td class="align-middle text-center">
                        @if ($section->meeting_link)
                            <a href="{{ $section->meeting_link }}" target="_blank"
                    ...
                    <td class="align-middle">
                        <button class="btn btn-sm btn-outline-info mb-0" data-toggle="modal"
